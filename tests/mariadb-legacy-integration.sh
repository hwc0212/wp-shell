...
test_root="$(mktemp -d)"
trap 'rm -rf -- "$test_root"; rm -f /etc/mysql/mariadb.conf.d/50-wordpress.cnf /etc/mysql/mariadb.conf.d/70-wp-shell-test-admin.cnf' EXIT
...
[[ "$(mariadb_snapshot_value "$unsafe" innodb_buffer_pool_size)" == 1073741824 ]]
[[ "$(mariadb_snapshot_value "$unsafe" max_connections)" == 300 ]]
